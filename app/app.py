...
def categoria(categoria_id):
    ...
    return render_template(
        "categoria.html", categoria=categoria, entradas=entradas
    )
    ...
    if request.method == "POST":
        ...
        if usuario_existente:
            ...
            nuevo_usuario = Usuario(
                username=username, password=password_hasheada
            )
            db.session.add(nuevo_usuario)
            db.session.commit()
            flash(
                "Registro exitoso. Inicia sesión con tus nuevas credenciales.",
                "success",
            )
